...
public class OrdenDeTrabajoControlador {
    ...
    public String agregarOrdenDeTrabajo(@RequestBody OrdenDeTrabajoRequest request) {
        ...
        if (request == null || request.getOrden() == null)
            throw new InvalidFieldException("La solicitud no contiene datos de la orden.");

        OrdenDeTrabajoDTO orden = request.getOrden();

        // Validación de IDs obligatorios
        if (orden.getId_cliente() <= 0 ||
                orden.getId_lugar() <= 0 ||
                orden.getId_trabajador() <= 0)
            throw new InvalidFieldException("Cliente, lugar o trabajador inválido.");

        // Validación de estado
        List<String> estadosPermitidos = List.of("PENDIENTE", "EN_PROCESO", "FINALIZADA");

        if (orden.getEstado() == null || !estadosPermitidos.contains(orden.getEstado()))
        ...
        for (DetalleOrdenTrabajoDTO d : detalles) {
            ...
            if (d.getDuracion() == null || d.getDuracion().isBlank())
                throw new InvalidFieldException("La duración del detalle es obligatoria.");
        }

        // Crear orden
        ordenDeTrabajoService.agregarOrdenTrabajo(orden);

        int idOrden = orden.getId_orden();
        ...
        return "Orden de trabajo y detalles creados correctamente.";
    }
